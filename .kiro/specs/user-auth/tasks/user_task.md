...
- [x] 修改specs中的requirement、task、design文档及其它相关文档，用户注册增加邮箱验证码验证功能：
...
- [x] 修改specs中的requirement、task、design文档及其它相关文档，优化邮箱验证码功能：
  - 在发送验证码前，校验该邮箱是否已注册，不允许已注册的邮箱重复注册
  - 在发送验证码前，校验邮箱是否有效
  - 增加发送邮箱验证码的其它异常处理

- [x] 修改specs中的requirement、task、design文档及其它相关文档，优化注册页面和登录页面：
  - 注册页面、登录页面在全屏显示的情况下，保持不出现竖向滚动条

- [x] 修改specs中的requirement、task、design文档及其它相关文档，优化界面及功能：
  - 基于MVVM的思想，重构页面，分离样式、dom结构及逻辑，提高页面加载速度及渲染效率，提高可维护性
  - 使用更加简洁高效、稳定可靠的方案，增强敏感信息的传输安全性，避免敏感信息泄露

- [x] 修改specs中的requirement、task、design文档及其它相关文档，优化邮箱验证码功能：
  - 输出注册页面使用的邮箱验证码模板到template目录中，由开发者维护到resend平台
  - 发送验证码使用resend平台上的指定模板，方便动态修改，避免硬编码
  - 使用专业的词汇及清晰的脉络，以markdown文档的形式，将邮件模板方案整理输出到docs目录中，要求格式统一、逻辑严谨
